...
Mesh::Mesh( const uint primCount )
{
	...
	texcoord0 = (float2*)_aligned_malloc(primCount * sizeof(float2), 64);
	texcoord1 = (float2*)_aligned_malloc(primCount * sizeof(float2), 64);
	texcoord2 = (float2*)_aligned_malloc(primCount * sizeof(float2), 64);
	normal0 = (float3*)_aligned_malloc(primCount * sizeof(float3), 64);
	normal1 = (float3*)_aligned_malloc(primCount * sizeof(float3), 64);
	normal2 = (float3*)_aligned_malloc(primCount * sizeof(float3), 64);
	memset(texcoord0, 0, primCount * sizeof(float2));
	memset(texcoord1, 0, primCount * sizeof(float2));
	memset(texcoord2, 0, primCount * sizeof(float2));
	memset(normal0, 0, primCount * sizeof(float3));
	memset(normal1, 0, primCount * sizeof(float3));
	memset(normal2, 0, primCount * sizeof(float3));
...
Mesh::Mesh( const char* objFile, const char* texFile )
{
	...
	texcoord0 = new float2[25000];
	texcoord1 = new float2[25000];
	texcoord2 = new float2[25000];
	normal0 = new float3[25000];
	normal1 = new float3[25000];
	normal2 = new float3[25000];
	...
	while (!feof( file ))
	{
		...
		if (line[0] != 'f') continue; else
			sscanf( line + 2, "%i/%i/%i %i/%i/%i %i/%i/%i",
				&a, &b, &c, &d, &e, &f, &g, &h, &i );
		tri[triCount].vertex0 = P[a - 1], normal0[triCount] = N[c - 1];
		tri[triCount].vertex1 = P[d - 1], normal1[triCount] = N[f - 1];
		tri[triCount].vertex2 = P[g - 1], normal2[triCount]= N[i - 1];
		texcoord0[triCount] = UV[b - 1], texcoord1[triCount] = UV[e - 1];
		texcoord2[triCount++] = UV[h - 1];
	}
	fclose( file );
	bvh = new BVH( this );
